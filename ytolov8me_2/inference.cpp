...
void ISEE(cv::Mat img)
{
    ...
    cv::Mat rawData = img;
    cv::Mat visualizeData = rawData.clone(); // 克隆原始数据以避免修改原始数据
    cv::minMaxLoc(visualizeData, &minVal, &maxVal); // 找到数据的最大值和最小值
    visualizeData = (visualizeData - minVal) / (maxVal - minVal) * 255; // 归一化到 [0, 255]
...
char* YOLO_V8::TensorProcess(clock_t& starttime_1, cv::Mat& iImg, N& blob, std::vector<int64_t>& inputNodeDims,
    ...
    Ort::Value inputTensor = Ort::Value::CreateTensor<typename std::remove_pointer<N>::type>(
        ...
        inputNodeDims.data(), inputNodeDims.size());//创建输入张量
#ifdef benchmark
    clock_t starttime_2 = clock();//用于存储系统时钟周期计数器的值
#endif // benchmark
    auto outputTensor = session->Run(options, inputNodeNames.data(), &inputTensor, 1, outputNodeNames.data(),
        outputNodeNames.size());//RUN推理
#ifdef benchmark
    clock_t starttime_3 = clock();
#endif // benchmark

    //获取模型输出张量的第一个张量的数据类型信息
    ...
    case YOLO_DETECT_V8_HALF:
    {
        // outputNodeDims[1]代表的是每个单元格输出的参数数量，包括边界框的坐标和类别置信度分数 的数目。
